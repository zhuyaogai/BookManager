...
public class LoginBiz {
     ...
     *
     *   我知道为什么会出错了。。。
     *      cookie会放在客户端中保存，每一次登录的时候都会带上server端生成的Ticket，但是
     *      浏览器保存的cookie也会有一个生存期限，过了这个期限，就会自动删除，于是再次登录的时候，拦截器就
     *      不能够获得用户信息而自动登录，所以当用户想登录的时候就必须重新想server端插入新的ticket，但是由于
     *      我们已经设置了对于每一个用户ticket必须唯一，所以就报错了。。
     *
     *      我想到的解决方法就是，在一段时间后，在server端也删除了 ticket
     *
     * @return 返回最新t票
